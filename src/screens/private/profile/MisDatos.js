...
const MisDatos = (props) => {
	...
	const getFondoGaleria = async () => {
		...
		if (status === 'granted') {
			...
			try {
				//subida a firebase
				const subida = await firebase.storage.ref().child('fondo').child(file.name).put(file, { contentType: file.type });
				//si la subida es exitosa 
				if (subida.state === 'success') {
					...
					// agregamos en la colecion la url
					await firebase.db.collection('usuarios').doc(docUsuario.id).update({ fondo: urlAvatar });
					// Snack que indique los cambios 
					setLoading(false);
					setSnackUpdate(true);
				}
